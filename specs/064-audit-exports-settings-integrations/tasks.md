# Tasks — Spec 064

- [x] T1 Close Spec 063 canonically and promote 064.
- [x] T2 Add utility view-model over doctor/disclosure/FHIR contracts.
- [x] T3 Implement Audit Trail.
- [x] T4 Implement Exports support matrix surface.
- [x] T5 Implement Settings readiness/non-claim surface.
- [x] T6 Implement Integrations default-deny/external-gate surface.
- [x] T7 Add regression tests and evidence.
- [x] T8 Run local qualification and MSRV/diff/headless checks.
- [ ] T9 Exact-head CI, merge, post-merge main verification, canonical close.
- [ ] T10 Promote Spec 065.
