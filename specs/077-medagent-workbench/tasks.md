...
- [ ] Read mandatory governance + Research OS V2 + Spec 077 authority chain.
...
- [ ] Confirm no live Spec 077 collision/superseding authority; prove
      numbering is free.
- [ ] Verify Spec 076 closure and post-main evidence.
...
- [ ] Freeze exact field-level `AgentIdentity`/`AgentProfile`/
...
- [ ] Add MedAgent storage using current encrypted vault/SQLite metadata
      architecture (v5 -> v6).
- [ ] Add required indexes (`migration.md` section 3).
- [ ] Implement atomic transactions (mutation + terminal-state/`RunReceipt`
      commit together) and stale-revision conflict behavior.
- [ ] Wire every 077 table into `SqliteMetaStore::snapshot_bytes` (schema
...
- [ ] Add forward migration test fixtures from representative pre-077
...
- [ ] Prove no 077 operation cascades deletion into canonical target
      objects.
- [ ] Fix forward any pre-existing test assertions in `project_graph_074.rs`/
