# Evidence — Spec 076 Collaboration Substrate

## Index

Required evidence set (see `specs/076-collaboration-substrate/plan.md`, section 076-K):
...
SECURITY_ADVERSARIAL.md (threat-gate results from security.md, incl. the T1 structural no-effect proof)
...
## Known workstation constraint (carried forward from Spec 075)

This workstation's local Windows toolchain cannot link (no MSVC `link.exe`/`cl.exe` installed; only Git's own non-MSVC `link.exe` is on `PATH`). Local `cargo build`/`cargo test` cannot produce a working binary. `cargo fmt --check` and `cargo metadata` remain available locally. GitHub Actions CI is the authoritative qualification path for every build/test/clippy/deny claim in this evidence packet; nothing here should be read as "verified by local execution" unless explicitly stated.
