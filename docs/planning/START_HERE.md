...
Spec **057** and Spec **058** are `CLOSED_CANONICAL`; Spec 058 qualified deterministic unsigned portable package + install/upgrade/rollback on Windows/Linux/macOS. Spec **059** is `POST_MERGE_REGRESSION_FIX_IN_REVIEW` for final repository-owned release closure/material-findings audit; PR #100 merged, but post-merge run `34892900032` exposed a Windows RSS parser defect; terminal closure is reopened for the bounded fix. Spec 012 remains optional/deferred. Advanced work is **060+** and not a Trusted V1 completion requirement.
