...
Specs **057**–**059** are `CLOSED_CANONICAL`, preserving Trusted V1 implementation closure. Spec **060** is also `CLOSED_CANONICAL` as the first separately promoted Desktop+CLI product-launch unit: PR #103 exact-head run `34909664814` and post-merge main run `34911182508` passed all six required jobs. Spec **061** is now `CLOSED_CANONICAL`: exact-head run `34916956439` and post-merge main run `34917999834` passed all six required jobs. Spec **062** is active. `RELEASE_READY=false`; Spec 012 remains optional/deferred.
