...
**Live follow-on status (2026-09-22):** Specs **016**–**077** are `CLOSED_CANONICAL` (see per-row table below for exact-head/merge/post-merge evidence); Spec **012** and Spec **036** are historical MESC interoperability provenance only under `MESC_PROJECT_SEPARATION.md`; MESC is a separate project and neither spec is current MedScale execution authority. Trusted V1, the separately promoted Desktop+CLI implementation phase, the Project + Artifact Graph foundation, the Data Source Fabric + Data Workbench foundation, the Collaboration Substrate, and the MedAgent Workbench are repository-complete pending external gates. Honesty: `PRIVATE_DATA_READY=false`; `RELEASE_READY=false`; `budgets_claimed_met=false`; sandbox `platform_qualified=false`.
...
| 075 | Data Source Fabric + Data Workbench Foundation | `CLOSED_CANONICAL` | Exact-head `9f84a6e…` passed run `35580861670`, PR #129 merged as `89a88cf…`, and post-merge main run `35582548200` passed all six required jobs. Required four exact-head CI attempts: two real Core/storage test-classification bugs fixed forward (`b6de6a2`, `bbc4d89`), then a full exact-range review (`/code-review high`) found and fixed 18 confirmed findings including a real authority-scope-bypass read escape and a credential-restore gap (`aa771e9`). Two honest, non-blocking gaps recorded, not hidden: remote dataset live-fetch never exercised beyond fail-closed deny-before-socket, and no rendered Desktop evidence exists (no CI rendering step, no headless `AppWindow` test harness). See `evidence/075-data-source-fabric/CLOSURE.md`. |
| 076 | Collaboration Substrate | `CLOSED_CANONICAL` | Exact-head `10c99f4…` passed run `35637687997`, PR #131 merged as `594c309…`, and post-merge main run `35640113781` passed all six required jobs. Required five exact-head CI attempts: a clippy doc-comment lint, a Rust borrow-checker error, and a real Desktop authorization bug (operator registered under the wrong holder id, every "Create Room" action would have failed) fixed forward, then an exact-range review using OpenCodeReview (delegation mode, per explicit instruction) found and fixed a real gap: `restore_v5` never re-verified the activity hash chain after replay (`04fa4cb`). Honest, non-blocking gaps recorded, not hidden: no rendered Desktop evidence exists (no CI rendering step), Notes/Approvals have no Desktop UI surface (CLI-only), and content-leakage proof (T11) is by inspection, not an automated log-capture test. See `evidence/076-collaboration-substrate/CLOSURE.md`. |
| 077 | MedAgent Workbench | `CLOSED_CANONICAL` | Exact-head `2ea886c…` passed run `35735800476`, PR #133 merged as `ce8a40d…`, and post-merge main run `35738904685` passed all six required jobs. Required an exact-range review using OpenCodeReview (delegation mode, per the founder's explicit standing directive) that found and fixed a real gap: `execute_search_context_artifacts` had no bound on per-artifact scan input size (`90589fc`). Two other real gaps were found and fixed by this spec's own tests during implementation, before any review pass: a `RunReceipt.tool_invocation_ids` provenance gap in the original `cancel_agent_run` (T077-08), and a UTF-8 char-boundary panic risk in the search tool's snippet extraction (T077-06). Honest, non-blocking gaps recorded, not hidden: no rendered Desktop evidence exists (no CI rendering step), agent identity registration/context-manifest creation are CLI-only (Desktop's run-creation form takes their ids as text entry), and content-leakage proof (T11) is by inspection, not an automated log-capture test. See `evidence/077-medagent-workbench/CLOSURE.md`. |
| 078 | Model Fleet + Compare | `PROMOTED_IMPLEMENTATION_AUTHORIZED` | Promoted 2026-09-22 on base `ff2e677…` (Spec 077 closure PR #134 exact-head run `35742088092` 6/6, post-merge main run `35745220872` 6/6). See `docs/planning/SPEC_078_PROMOTION.md` and `specs/078-model-fleet-compare/`. Not yet implemented. |
| 079+ | Advanced deferred work | `DEFERRED_BY_CANONICAL_DESIGN` | Research OS V2 candidates 079+ (Privacy Gate, Governed Browse + Medical Literature Acquisition, AudioFlow + Local Scribe, Analytics Gate + Cohort Builder, Clinical Graph + Knowledge/Research Canvas, Hub, Compute, R Workspace, Community Extensions, AudioFlow Advanced, Research/Evidence Packs, Institutional Adapters, Federation, Whole-Platform Qualification); require fresh promotion after 078 closes. Note: 079's own hard dependency is 074+077 only (078 is not a hard prerequisite per `RESEARCH_OS_V2_SPEC_IMPLEMENTATION_CONTRACTS.md`), so 079 could in principle promote independently of 078's closure; 078 is promoted first per this repository's established strict numeric-order convention. |
...
**Next eligible (honest):** Specs **068**-**077** are `CLOSED_CANONICAL`. Spec **078** (Model Fleet + Compare) is `PROMOTED_IMPLEMENTATION_AUTHORIZED` as of 2026-09-22; `NEXT_PROMOTED_SPEC=078`. Implementation has not yet started (`specs/078-model-fleet-compare/tasks.md` T078-00 not yet complete). Candidates 079+ remain `DEFERRED_BY_CANONICAL_DESIGN` and require a fresh promotion package before any implementation is authorized. `RELEASE_READY`, `PRIVATE_DATA_READY`, and `MULTI_CLIENT_RELEASE_READY` remain false. MESC is a separate project/repository and is excluded from MedScale execution, completion, and release criteria; historical Specs 012/036 remain provenance only.
