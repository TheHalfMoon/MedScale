...
**Live follow-on status (2026-09-21):** Specs **016**–**076** are `CLOSED_CANONICAL` (see per-row table below for exact-head/merge/post-merge evidence); Spec **012** and Spec **036** are historical MESC interoperability provenance only under `MESC_PROJECT_SEPARATION.md`; MESC is a separate project and neither spec is current MedScale execution authority. Trusted V1, the separately promoted Desktop+CLI implementation phase, the Project + Artifact Graph foundation, the Data Source Fabric + Data Workbench foundation, and the Collaboration Substrate are repository-complete pending external gates. Spec **077** (MedAgent Workbench) is canonically promoted and implementation is in progress on `spec/077-medagent-workbench`. Honesty: `PRIVATE_DATA_READY=false`; `RELEASE_READY=false`; `budgets_claimed_met=false`; sandbox `platform_qualified=false`.
...
| 077 | MedAgent Workbench | `PROMOTED_IMPLEMENTATION_AUTHORIZED` | Live truth reverified 2026-09-21 (Spec 076 closure confirmed: PR #131 exact-head run `35637687997` 6/6, merged `594c309…`, post-main run `35640113781` 6/6; PR #132 exact-head run `35642851086` 6/6, merged `80aefae…`). Dependency graph confirms hard dependency on 074 only (`CLOSED_CANONICAL`); integration with 076's `ParticipantKind::Agent`/`AgentParticipantIdentity` is available since 076 is also closed. Numbering verified free. Promoted on base `80aefae…`; see `SPEC_077_PROMOTION.md` and `specs/077-medagent-workbench/`. Implementation in progress on `spec/077-medagent-workbench`. |
...
**Next eligible (honest):** Specs **068**-**075** are `CLOSED_CANONICAL`. Spec **076** (Collaboration Substrate) is `PROMOTED_IMPLEMENTATION_AUTHORIZED` as of 2026-09-21 and implementation is in progress; `NEXT_PROMOTED_SPEC=076`. Candidates 077+ remain `DEFERRED_BY_CANONICAL_DESIGN` and require a fresh promotion package (dependency-ready candidate to be determined from the live Research OS plan at that time) before any implementation is authorized. `RELEASE_READY`, `PRIVATE_DATA_READY`, and `MULTI_CLIENT_RELEASE_READY` remain false. MESC is a separate project/repository and is excluded from MedScale execution, completion, and release criteria; historical Specs 012/036 remain provenance only.
