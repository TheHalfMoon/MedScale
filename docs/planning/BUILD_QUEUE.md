...
**Live follow-on status (2026-09-21):** Specs **016**–**075** are `CLOSED_CANONICAL` (see per-row table below for exact-head/merge/post-merge evidence); Spec **012** and Spec **036** are historical MESC interoperability provenance only under `MESC_PROJECT_SEPARATION.md`; MESC is a separate project and neither spec is current MedScale execution authority. Trusted V1, the separately promoted Desktop+CLI implementation phase, the Project + Artifact Graph foundation, and the Data Source Fabric + Data Workbench foundation are repository-complete pending external gates. Spec **076** (Collaboration Substrate) is canonically promoted and implementation is in progress on `spec/076-collaboration-substrate`. Honesty: `PRIVATE_DATA_READY=false`; `RELEASE_READY=false`; `budgets_claimed_met=false`; sandbox `platform_qualified=false`.
...
| 076 | Collaboration Substrate | `PROMOTED_IMPLEMENTATION_AUTHORIZED` | Live truth reverified 2026-09-21 (Spec 075 closure confirmed: PR #129 exact-head run `35580861670` 6/6, merged `89a88cf…`, post-main run `35582548200` 6/6; PR #130 exact-head run `35585345326` 6/6, merged `6021ff9…`, post-main run `35587568653` 6/6). Dependency graph confirms hard dependency on 074 only (`CLOSED_CANONICAL`); numbering verified free. Promoted on base `6021ff9…`; see `SPEC_076_PROMOTION.md` and `specs/076-collaboration-substrate/`. Implementation in progress on `spec/076-collaboration-substrate`. |
