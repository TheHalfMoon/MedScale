...
**Next eligible (honest):** Specs **068**-**077** are `CLOSED_CANONICAL`. No spec is currently `PROMOTED_IMPLEMENTATION_AUTHORIZED`; `NEXT_PROMOTED_SPEC=none`. Candidates 078+ remain `DEFERRED_BY_CANONICAL_DESIGN` and require a fresh promotion package (dependency-ready candidate to be determined from the live Research OS plan at that time) before any implementation is authorized. `RELEASE_READY`, `PRIVATE_DATA_READY`, and `MULTI_CLIENT_RELEASE_READY` remain false. MESC is a separate project/repository and is excluded from MedScale execution, completion, and release criteria; historical Specs 012/036 remain provenance only.
