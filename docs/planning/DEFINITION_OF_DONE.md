...
If code is complete but final UI/app-store/partner/legal/real-PHI gates remain, report `IMPLEMENTATION_COMPLETE_PENDING_EXTERNAL_GATES` rather than pretending those gates passed. MESC is a separate project/repository and is excluded from MedScale completion and release criteria; historical interoperability evidence does not create current scope. Spec 016+ is not required for V2 completion unless later promoted by canonical evidence.
