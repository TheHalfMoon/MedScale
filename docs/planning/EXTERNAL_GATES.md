...
| WORKER_OS_SANDBOX_PLATFORM_QUALIFIED | `OPEN` | Landlock / AppContainer / Seatbelt PLATFORM_QUALIFIED for workers | Spec 026 ships Linux Landlock **ReadyBaseMeasured** apply + measured deny evidence; Windows/macOS remain NotPlatformQualified scaffolds. Multi-OS PLATFORM_QUALIFIED evidence still required before clearing this gate. Do **not** treat ReadyBaseMeasured as gate close. |
| MESC_RELEASED_ARTIFACT | `NOT_AVAILABLE` | Spec 012 immutable MESC artifact (hash/rights/SBOM) | Re-check 2026-08-26: MESC main `4b193c01Â` TRAINING_CODE_READY=YES but RELEASE_STATUS=BLOCKED; v0.1.0 release_id 352847712 assets=[]; v0.2.0 tag has no Release. See evidence/012-mesc-artifact-integration/GATE_CHECK.md. Fail-closed MescArtifactAdmit ships; do not clear gate. |
