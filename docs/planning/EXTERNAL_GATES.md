...
| REPO_BRANCH_PROTECTION_REQUIRED_CHECKS | `NOT_CONFIGURED_OWNER_SETTINGS` | GitHub branch protection, required status checks, and related repository rulesets for `main` | Spec 022 documents the gate. Spec **037** ships exact required-check inventory at `evidence/022-release-qualification-prep/REQUIRED_CHECKS.md` (rust ubuntu/windows/macos + cargo-deny + supply-chain policy present). Do **not** change repository settings via API. Owner must configure protection/required checks; Cursor continues independent engineering and must not claim RELEASE_READY. |
