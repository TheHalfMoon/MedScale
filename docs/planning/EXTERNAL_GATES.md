...
| FINAL_V0_UI_ACCESSIBILITY_QUALIFICATION | `BLOCKED_BY_FINAL_V0_UI_ARTIFACT` | Final v0 visual UI and WCAG qualification | Fixture semantics are qualified, but `imports/v0/` has no final UI artifact. External action: supply final v0 artifact; then run final accessibility/WCAG qualification. This maps `wcag_final_v0_ui_accessibility_qualification`. |
